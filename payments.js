...
function registerPaymentRoutes(app, { admin, authMiddleware, callerIsAdmin, express }) {
  ...
  async function applyWalletTx({
    ...
    if (referenceId) {
      const dup = await db
        ...
        .get();
      if (!dup.empty) {
        return Number(dup.docs[0].data().balanceAfter || 0);
      }
    }
  ...
  function stripeProductName(purpose) {
    switch (purpose) {
      case "wallet_top_up":
        return "Milkman App Credits";
      case "appointment":
        return "Milkman Vet Consultation";
      case "subscription":
        return "Milkman Milk Subscription";
      default:
        return "Milkman Order";
    }
  }
  ...
  app.post("/api/wallet/pay", authMiddleware, async (req, res) => {
    try {
      const { amount, referenceId, note, type } = req.body ?? {};
      ...
      const balanceAfter = await applyWalletTx({
        userId: req.user.uid,
        amount: -debit,
        type: typeof type === "string" ? type : "purchase",
        note: note || "Purchase",
        referenceId: referenceId || null,
        createdBy: req.user.uid,
      });
        ...
        line_items: [
          {
            price_data: {
              ...
              product_data: {
                name: stripeProductName(purpose),
              },
            },
            quantity: 1,
          },
